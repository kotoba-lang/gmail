(ns gmail.app
  "etzhayyim-wasm-gmail-gm4il0x1 appview — reagent + re-frame, view built from
  ...
  - `:app/xrpc?` stays `true` — but for a different reason than the value
    itself might suggest. Before this migration, the deployed Worker was
    the SvelteKit adapter build, and the XRPC handler `+page.svelte` was
    describing was `svelte/src/routes/xrpc/[...path]/+server.ts` (moved,
    not deleted, to `src/xrpc-proxy.ts` — see that file's header; it
    proxies to `AGENTGATEWAY_MCP_ROUTER_URL` and is NOT wired into this
    migration's wrangler.jsonc `main`). After this migration, `main` is
    `src/app.ts` — this repo's actual production Worker facade (see its
    own header and this repo's CLAUDE.md / kotodama.jsonld
    `component.path`) — which handles `/xrpc/com.etzhayyim.apps.gmail.*`
    directly by proxying to `DISPATCHER_URL`, and which was already
    calling `env.ASSETS.fetch(req)` as its fallback branch before this
    migration touched anything. So XRPC really is live after this
    migration, just served by a different, already-existing file than
    the one this status page used to point at.
  ...
  components or ext-rules change) with:
    ...
    (spit \"public/index.html\"
          (page/->page {:title \"etzhayyim-project-gmail\"
                         :lang \"ja\"
                         :description \"gmail — etzhayyim Gmail Intelligence appview facade status page (reagent + re-frame + jp-go-dds).\"
                         :css (slurp (io/resource \"jp_go_dds/dds.css\"))}
                        [:div {:id \"app\"} \"etzhayyim-project-gmail loading…\"]
...
(def default-db
  {:app/title "Gmail Gm4il0x1"
   :app/project "etzhayyim-project-gmail"
   :app/name "etzhayyim-wasm-gmail-gm4il0x1"
   :app/kind "appview"
   :app/route-count 0
   :app/routes []
   :app/vars []
   :app/xrpc? true
   :app/relative-path "cljs/src/gmail/app.cljs"})
