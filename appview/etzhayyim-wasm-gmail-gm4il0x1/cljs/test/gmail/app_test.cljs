...
(deftest initialize-db-sets-defaults
  (testing ":initialize-db populates every fact the Svelte scaffold held"
    ...
    (is (= "Gmail Gm4il0x1" @(rf/subscribe [:app/title])))
    (is (= "etzhayyim-wasm-gmail-gm4il0x1" @(rf/subscribe [:app/name])))
    (is (= "etzhayyim-project-gmail" @(rf/subscribe [:app/project])))
    ...
    (is (= "cljs/src/gmail/app.cljs" @(rf/subscribe [:app/relative-path])))))
